    ...
    <section class="content"> 
      ...
      <div class="box"> 
        ...
        <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <select id="nama" class="form-control" required>
                    ...
                    <?php foreach ($nama_data as $key): ?>
                      <option value="<?=@$key['premix_kode']?>"><?=@$key['premix_nama'].' [ '.@$key['premix_kode'].' ]'?></option>
                    <?php endforeach ?>
                  </select>
                </div>
              </div>
          ...
          <div class="col-md-12 col-om premix_sub" id="premix_sub">
...
$(document).on('change', '#nama', function() {
  ...
  //reset before append
  $("#premix_sub").load(" #premix_sub > *", function(){

    //action
    $('form').attr('action', '<?=base_url('produksi/premix_tambah_stok/')?>'+kode);

    $.get('<?=base_url('produksi/premix_get_premix/')?>'+kode, function(data) {
      ...
      $.each(json, function(index, val) {
        ...
        //insert value
        $('#copy:nth-child('+i+') > td:nth-child(1) > select').val(val.premix_campur_barang).change();
        $('#copy:nth-child('+i+') > td:nth-child(3) > div > input').val(val.premix_campur_qty);
...
$(document).on('change', '#premix, #obat', function() {
